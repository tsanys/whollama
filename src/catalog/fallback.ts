...
export async function loadCuratedCatalog(): Promise<OllamaModel[]> {
  try {
    const data = await fs.readFile(FALLBACK_PATH, 'utf-8')
    const models: OllamaModel[] = JSON.parse(data)
    return models.map((m) => ({ ...m, source: 'curated' as const }))
  } catch {
...
function getMinimalFallback(): OllamaModel[] {
  return [
    {
      name: 'llama3.2:3b',
      family: 'llama3.2',
