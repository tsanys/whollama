 ...
 */
export async function fetchArenaElo(): Promise<Map<string, number>> {
  const endpoints = [
...
async function tryFetchEndpoint(url: string): Promise<Map<string, number>> {
  try {
    ...
    if (Array.isArray(data)) {
      const eloScores: Array<{ name: string; score: number }> = []

      for (const entry of data) {
        if (typeof entry !== 'object' || entry === null) continue
        const e = entry as Record<string, unknown>
        const name = (e.model ?? e.model_name ?? e.name) as string | undefined
        const elo = (e.elo_score ?? e.elo ?? e.score) as number | undefined
