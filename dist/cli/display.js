...
function renderHardwareBox(hardware) {
...
export function renderTable(results, hardware, task) {
    console.log();
    ...
    console.log();
    console.log(`  ${chalk.green('Top pick:')} ${results[0]?.name ?? 'N/A'}  •  ${chalk.dim(`Run: ${results[0]?.pull_command ?? ''}`)}`);
    console.log();
}
export function renderJson(results, hardware) {
    const output = {
        hardware: {
            gpu: {
...
export function renderModelInfo(model) {
    const lines = [];
