...
export async function detectNvidiaGpu() {
    try {
        const { stdout } = await execa('nvidia-smi', ['--query-gpu=name,memory.total', '--format=csv,noheader,nounits'], { timeout: 10000 });
        const line = stdout.trim();
        if (!line)
            return null;
        // Format: "NVIDIA GeForce RTX 4090, 24576"
        const [nameRaw, vramMbRaw] = line.split(',').map((s) => s.trim());
        const vramMb = parseInt(vramMbRaw, 10);
        if (!nameRaw || isNaN(vramMb))
            return null;
